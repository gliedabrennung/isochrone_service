...
|---|---|---|---|
| 11.2 | dev-профиль в `docker-compose.override.yml` | файл есть, но `.env` задаёт `COMPOSE_FILE=docker-compose.yml` | compose подхватывает `override.yml` автоматически. Без этого «обычный» `docker compose up -d` публиковал бы наружу порты api/valhalla/redis, нарушая п. 10.6 и AC-18 |
| 10.4 | `api` зависит от `valhalla` с `condition: service_healthy` | `condition: service_started` | п. 10.4 одновременно требует, чтобы во время сборки тайлов API отвечал `503 ENGINE_UNAVAILABLE`. С `service_healthy` API не запустился бы вовсе и клиент получал бы 502 от nginx. Готовность движка проверяется в `/ready` и на каждом запросе |
...
| 6.4 | `/ready` возвращает 503 при недоступности Redis или Valhalla | 503 только при недоступности Valhalla, Redis даёт `status: degraded` | п. 10.3 и AC-12 требуют, чтобы остановка Redis не приводила к отказу сервиса. Детализация по компонентам в ответе сохранена |
